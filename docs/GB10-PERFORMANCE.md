...
- **Measured single-stream speedup on GB10: `+21–24%` over no-spec autoregressive**
  (26.7 t/s no-spec → 32.4 t/s DSpark, apples-to-apples). This is **modest, and honest** —
...
- **Absolute single-stream t/s is gated by raw forward compute**, not interconnect. GB10's
  **273 GB/s** unified LPDDR5X is **~12× less** than H100's 3.35 TB/s HBM3. A full 43-layer
  ...
  size** — measured: `block_size 5 → 4` changed throughput by ~0 (32.4 → 31.4 t/s), proving
...
- **DSpark's real, defensible win on GB10 is concurrency:** **+55%** aggregate over our
  previous MTP-2 production at 8 concurrent users (real-world ~99 vs ~64 t/s), measured at
  the tuned prod config `--max-num-seqs 12` (sustains **141–146 t/s** at 12 concurrent).
- **Single-stream is gated by the fixed forward, not the draft.** `block_size 5 → 4`
  changed throughput by ~0 (32.4 → 31.4 t/s), so there is no block-size lever to push past
  it. 40+ t/s single-stream is **not reachable** on this hardware without NVLink-class step
...
| **B** | **Hardware = compute/bandwidth, not interconnect.** GB10 = 273 GB/s unified LPDDR5X (~12× less BW than H100 HBM3). The verify+draft forward is **~65 ms/step, compute-bound**. The 2-node RoCE TP all-reduce is a **red herring** (~4 MiB/step, <3.5% of the step time — measured, not assumed). DeepSeek benched on H100/H800 → far cheaper step → higher absolute t/s *and* a bigger % over their weak baseline. | **~30%** | live profile: all-reduce <2.3 ms of a 65 ms step; GB10 273 GB/s vs H100 3.35 TB/s |
...
| **D** | **Draft efficiency.** Our vLLM fp8-MLA + 256-expert-MoE draft vs DeepSeek's hand-tuned tilelang kernels. But the draft is only **~15% of the step** — even a *free* draft caps single-stream at ~37 t/s. | **~10%** | profile: draft ~10–12 ms of a ~65 ms step |
...
- mean_acceptance ≈ **2.2–2.5**  → **32.4 t/s** single-stream measured
...
To reach **40 t/s** you would need mean_acceptance ≈ **2.6 at the current step** *or* a
step −17%. Acceptance ≥ reference (pos0 ≥0.76 sustained) is only seen on easy/code content,
and the step is a *fixed* forward (block size is not a lever — see below). **40+ t/s
...
|---|---|---|
| concurrency c8  | 69 t/s | **99 t/s** (**+44%**) |
| concurrency c12 | — | **141–146 t/s** (sustained 3×) |
| concurrency c16 | 75 t/s | **117 t/s** (**+56%**) |

vs the previous **MTP-2** production (real ~64 t/s at 8 streams), DSpark seqs=12 at c8
≈ **99 t/s = +55%** (it was +27% at seqs=4). Health under load: acceptance **2.3–2.56**,
**0 crashes**, no OOM, `RestartCount 0`.

| | DSpark (seqs=12) | prev prod (MTP-2) |
|---|---|---|
| concurrency c8 (real, measured) | **~99 t/s** | ~64 t/s (**+55%**) |
| single-stream | 32.4 t/s | faster (lighter draft) |
| acceptance pos0 | 0.62–0.74 (≈ reference) | — |
| context | 1M (YaRN) | 1M |

> The ~64 t/s MTP-2 figure is the **real** 8-stream throughput a user measured in
> production, not a synthetic single-config bench. DSpark's ~99 t/s is the same kind of
...
- **`--max-num-seqs 12` is the tuned prod config — *not* 4.** An earlier warning said "do
  ...
  OOM, `RestartCount 0`. seqs=12 sustains 141–146 t/s at 12 concurrent streams. (Don't push
...
|---|---|---:|
| **No-spec autoregressive** (V4-Flash, spec off) | **26.7 t/s** | the honest single-stream baseline |
| **DSpark** (block_size=5, prod config) | **32.4 t/s** | **+21–24% over no-spec** |
| **DSpark** block_size 5 → 4 | **31.4 t/s** | **no gain** — the step is the fixed forward |
| **DSpark concurrency** (seqs=12), 8 streams | **~99 t/s** | vs ~64 t/s prior MTP-2 prod = **+55%** |
| **DSpark concurrency** (seqs=12), 12 streams | **141–146 t/s** | sustained 3× |
| **DSpark concurrency** (seqs=12), 16 streams | **117 t/s** | vs 75 t/s at seqs=4 = **+56%** |
...
tokens did *not* speed up single-stream decode (32.4 → 31.4 t/s, i.e. flat / slightly
...
|---|---:|---:|
| **no-spec autoregressive** | 26.7 t/s | 26.6 t/s |
| **single-stream, DSpark, short ctx** | 32.4 t/s | 55–63 t/s |
| **mean draft-accept length** | ~2.1 | 3.0–4.2 |
| **C16-static concurrency (200K)** | — (not built for this profile) | ~315–324 t/s |

**The mechanism is the interesting part, and it is *not* bandwidth.** No-spec decode is
**identical** across the two (26.7 vs 26.6 t/s) — the smaller NVFP4 KV cache buys nothing on the
